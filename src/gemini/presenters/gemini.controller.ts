...
export class GeminiController {
  ...
  getChatByTeacher(@Query('teacherId') teacherId: string) {
    if (!teacherId) throw new BadRequestException('teacherId가 필요합니다.');
    return {
      teacherId,
  ...
  async generateText(
  ...
  ): Promise<GenAiResponse & { teacherId: string }> {
    ...
    if (!teacherPrompts[dto.teacherId]) {
      throw new BadRequestException(
        `유효하지 않은 teacherId: ${dto.teacherId}. 가능한 값은 teacher1, teacher2, teacher3, teacher4, teacher5, teacher6입니다.`,
      );
    }
